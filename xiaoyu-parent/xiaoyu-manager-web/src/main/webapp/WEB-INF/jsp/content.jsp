	...
	tree.tree({ // 创建树
		...
		onClick : function(node){
			...
			if(tree.tree("isLeaf",node.target)){
				// 加载表中的数据 重新发送URL请求 加载数据
				datagrid.datagrid('reload', {
					// 发送的参数：categoryId 分类的id
					categoryId :node.id
		        });
			}
		}
...
var contentListToolbar = [{
    ...
    handler:function(){
		...
		// if(node)  表示有节点
		// $("#contentCategoryTree").tree("isLeaf",node.target) 获取节点，并判断是否为叶子节点  true表示为叶子节点
		...
		// 创建窗口
    	TT.createWindow({
			// 动态发送url请求：/content-add  获取到JSP页面，展示在窗口
			url : "/content-add"
		}); 
    }
},{
