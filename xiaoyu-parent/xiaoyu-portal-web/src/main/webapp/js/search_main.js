...
SEARCH.util = {htmlspecialchars: function(a, b) {
        ...
        var c = new RegExp("(^|\\?|&)" + b + "=([^&]*)(\\s|&|$)", "impl"), d = a ? a : window.location.search;
        if (c.test(d)) {
            return RegExp.$2
        }
        return ""
...
SEARCH.get_book_summary = {fragment: ['<div class="img"><img src="{img}" width="280" height="280" alt="" /></div><div class="text"><table cellpadding="0" cellspacing="0" border="0" width="100%">{detaile}</table></div>', '<div class="summary"><div class="impl-summary"><div class="close" onclick="$(this).parent().parent().hide()"></div><div class="arrow"></div><div class="con"><div class="iloading">\u6b63\u5728\u52a0\u8f7d\u4e2d\uff0c\u8bf7\u7a0d\u5019...</div></div></div></div>', "<tr><td>{name}{value}</td></tr>"],dataUrl: "http://d.360buy.com/bookExt/get?skuId=",edataUrl: "http://e.jd.com/ebook/ExteBookService.aspx?stype=search&skuid=",setPosition: function(a) {
...
SEARCH.get_im_info = function(a, b) {
    ...
    $.ajax({url: "http://chat1.jd.com/api/checkChat?venderList=" + b,dataType: "jsonp",jsonp: "callback",scriptCharset: "utf-8",success: function(m) {
            ...
            for (var k = 0, h = a.length; k < h; k++) {
                ...
                if (s.code == 1) {
                    o = '<impl class="online" title="联系客服"></impl>'
                } else {
                    if (s.code == 3) {
                        o = '<impl class="offline" title="客服留言"></impl>'
                    }
                }
                ...
                q.find("a").html(q.find("a").html() + o).find("impl").click((function(i) {
                    return function() {
                        window.open(i);
                        return false
                    }
...
SEARCH.top_page_html = function(d, e, g, b) {
    ...
    var c = '<span class="text"><impl>' + d + "</impl>/" + e + "</span>";
    if (d <= 1) {
...
SEARCH.get_presale_info = function() {
    ...
    b.each(function() {
        ...
        $.getJSON("http://soa.yushou.jd.com/yushoulistinfo.action?sku=" + g + "&callback=?", function(o) {
            ...
            var q = o.ret, l = q.d;
            var n = q.s == "0" ? "未开始" : "预售中";
            var s = '<div class="prebuy-time" time="' + l + '"><div class="pt-bg"></div><impl></impl><span>' + n + "</span><em>" + c(l) + "</em></div>";
            h.find("div.p-img").find("a").append(s);
            if (q.cp) {
                h.find("div.p-price").find("strong").html("￥" + q.cp)
            }
            if (q.cc) {
                h.find("div.p-price").append('<impl class="prebuy-num">（已有' + q.cc + "人预定）</impl>")
            }
            if (q.t == "2" && q.sa) {
                ...
                for (var p = 0, m = q.sa.length; p < m; p++) {
                    ...
                    k += "<strong>￥" + q.sa[p].m + '</strong></a><impl class="bottom"><em></em></impl></span>'
                }
                k += "</div>";
...
SEARCH.get_paipai_ware = function(c) {
    ...
    $.ajax({url: "http://sse1.paipai.com/comm_json?charset=utf-8&dtype=jsonp&dtag=search&KeyWord=" + b + "&PageSize=5&ShowClass=1",dataType: "jsonp",scriptCharset: "utf-8",success: function(h) {
            ...
            var g = '<div class="m m0 paipai-result"><div class="mt"><h2 class="paipai"><impl></impl>为您在拍拍找到如下结果</h2></div>';
